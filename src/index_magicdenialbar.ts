...
import { MagicDenialBar } from "./logic/magicDenialBar";
...
setupLogging("magicdenialbar");

let conn: API_Connector | null = null;
let testLogic: MagicDenialBar | null = null;
...
async function run() {
	initMetrics(8010, "magicdenialbar");

	const acc = accounts.magicdenial as [string, string];
	...
	testLogic = new MagicDenialBar(conn);
	...
	await conn.ChatRoomJoinOrCreate(
		{
			Name: "Magic Denial Bar",
			Description: "[BOT] get denied, ordered and played with | version 2022-12-02",
